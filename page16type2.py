...
# page16 type2
def detect_word_location0(img, word, length, threshold=95):
...
if __name__ == "__main__":
    ...
    for pdf_path in ['RECERT1.pdf']:
        ...
        #hight:
        Height_ROI=detect_word_location0(image_org,'Height:',100)
        if Height_ROI:
        ...
        #Weight:
        Weight_ROI=detect_word_location0(image_org,'Weight:',100)
        if Weight_ROI:
        ...
        #other:
        other_ROI=detect_word_location(image_org,'other',410)
        if other_ROI:
        ...
        requirements_ROI=detect_phrase_location(image_org, 'Nutritional requirements', 480)
        if requirements_ROI:
            ...
            requirements_ROI=detect_phrase_location(image_org, 'Nutritionalrequirements', 300)
            if requirements_ROI:
                x1,y1,x2,y2=requirements_ROI[0]
                roi=x1,y1,x2,y2
                requirements_text=extract_text_from_roi(image_org,roi)
