...
def preprocess_image_and_smouth_lines_for_drop_arrow(image_org,ratio):
    ...
    kernel_length = np.array(binary).shape[1]//1000  # Define the kernel size
...
def detect_word_location(img, word, length, threshold=80):
    ...
    for line in hocr_data.splitlines():
        ...
        if fuzz.partial_ratio(line, word) >= threshold:
            ...
            bounding_boxes.append((x1+92, y1, x1+92+length, y2))
...
def extract_numbers_from_roi(image_org, roi_coordinate):
    ...
    denoised = cv2.fastNlMeansDenoisingColored(roi_image, None,1, 1, 7, 21)
    # Image.fromarray(denoised).show()
    number = pytesseract.image_to_string(denoised, config='--psm 6 outputbase digits').strip()
    return number
...
if __name__ == "__main__":
    ...
    for pdf_path in ['BAD_QUALITY.pdf','BAD_QUALITY_3.pdf','BAD_QUALITY.pdf','FILLABLES_2.pdf','FILLABLES_3.pdf','FILLABLES_4.pdf','FILLABLES_5.pdf','FILLABLES_6.pdf','FOR_UPWORK_#1.pdf','FOR_UPWORK_#2.pdf','FOR_UPWORK_#3.pdf','FOR_UPWORK_#4.pdf','HIGH_QUALITY_2.pdf','HIGH_QUALITY_3.pdf','HIGH_QUALITY.pdf','MEDIUM_QUALITY.pdf']:
        excel_inputs={}
        print(pdf_path)
        ...
        if pdf_path=='FOR_UPWORK_#4.pdf':
            ...
            plure=True
            skewed=True
        elif pdf_path=='HIGH_QUALITY_3.pdf':
            skewed=True
        ...
        thresh_box_ratio=80#kanet 120 bekri w kanet temchi m3a kamel les pdf li semouhom for upwork
        ...
        can=True
        image_org, boxes = detect_checkboxes(thresh_check,image_org)
        image_org, drop_arrow = delet_drop_arrow(thresh_box,image_org)
        # Image.fromarray(image_org).show()
        # if len(arrows)<2:
        #     print('second try+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        #     thresh_box = preprocess_image_and_smouth_lines2(image_org,thresh_box_ratio)
        #     image_org, arrows = detect_box(thresh_box,image_org)
        # # Image.fromarray(image_org).show()
