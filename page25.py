...
#page25
def detect_checkboxes0(thresh,img):
    output = img.copy()
    contours, hierarchy = cv2.findContours(thresh, 1, 2)
...
def detect_word_location_old_method(img,word,length):
    ...
    return bounding_boxes
#page25
def detect_phrase_location(img, phrase, length, threshold=75):
    hocr_data = pytesseract.image_to_pdf_or_hocr(img, extension='hocr').decode('utf-8')
    words = phrase.split()
    bounding_boxes = []
...
def extract_text_from_roi_checks(image_org, roi_coordinates):
    ...
    for roi_x_start, roi_y_start, roi_x_end, roi_y_end in roi_coordinates :
        ...
        if not(first_word=='nutritional' and 'cope'==text.split(' ')[2]):
...
if __name__ == "__main__":
    start_time = time.time()
    for pdf_path in ['FILLABLE-10.pdf','BAD_QUALITY_3.pdf','FILLABLE-9.pdf','BAD_QUALITY_3.pdf','BAD_QUALITY.pdf','FILLABLES_2.pdf','FILLABLES_3.pdf','FILLABLES_4.pdf','FILLABLES_5.pdf','FILLABLES_6.pdf','FOR_UPWORK_#1.pdf','FOR_UPWORK_#2.pdf','FOR_UPWORK_#3.pdf','FOR_UPWORK_#4.pdf','HIGH_QUALITY_2.pdf','HIGH_QUALITY_3.pdf','HIGH_QUALITY.pdf','MEDIUM_QUALITY.pdf']:
        print(pdf_path)
        ...
        output_image, boxes = detect_checkboxes0(thresh_check,output_image)
        ...
        requirements_ROI=detect_phrase_location(image_org, 'Nutritional requirements', 410)
        if requirements_ROI:
            ...
            requirements_ROI=detect_phrase_location(image_org, 'Nutritionalrequirements', 480)
            if requirements_ROI:
                x1,y1,x2,y2=requirements_ROI[0]
                roi=x1,y1,x2,y2
                requirements_text=extract_text_from_roi(image_org,roi)
